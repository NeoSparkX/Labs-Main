...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  // Cache static offsets relative to scroll container/document
  ...
  // Measures and caches the static offsets by temporarily setting relative position
  const measureOffsets = useCallback(() => {
    const scroller = scrollerRef.current;
    const cards = cardsRef.current;
    if (!cards.length) return;

    // 1. Temporarily clear transforms and set position to relative to get clean static layout positions
    const cachedPositions = cards.map(c => c.style.position);
    const cachedTransforms = cards.map(c => c.style.transform);
    ...
    // Force layout reflow
    void (scroller?.offsetHeight || document.body.offsetHeight);

    const endElement = (useWindowScroll
      ? document.querySelector('.scroll-stack-end')
      : scroller?.querySelector('.scroll-stack-end')) as HTMLElement | null;

    // 2. Measure static positions
    ...
    cards.forEach((card, i) => {
      card.style.position = cachedPositions[i];
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      // Note: card is placed at cardTop. Trigger scaling when it reaches stackPositionPx
      ...
      if (hasChanged) {
        // Only scale/rotate since translation is natively handled by CSS position: sticky
        ...
        lastTransformsRef.current.set(i, { translateY: 0, ...newTransform });
      }
  ...
  useLayoutEffect(() => {
    ...
    cards.forEach((card, i) => {
      ...
      card.style.setProperty('-webkit-transform', 'scale(1) translateZ(0)');
    ...
    // Initial measurement after a brief paint timeout to make sure DOM is fully ready
    const initTimer = setTimeout(() => {
      measureOffsets();
      updateCardTransforms();
    }, 150);

    // Set up a ResizeObserver to observe size changes (like image load layout shifts)
    // and automatically update cached coordinates
    const targetToObserve = scroller || document.documentElement;
