...
class App {
  ...
  isDown = false;
  start = 0;
  clickStart?: { x: number; y: number; time: number };
  raf = 0;
  ...
  constructor(
    ...
    this.originalLength = items?.length || 1;
  ...
  onTouchDown(e: any) {
    ...
    this.clickStart = { x: this.start, y: e.touches ? e.touches[0].clientY : e.clientY, time: Date.now() };
  }
  ...
  onTouchUp(e: any) {
    ...
    if (this.clickStart) {
      ...
      const time = Date.now() - this.clickStart.time;
      // Allow click if moved less than 5px and clicked under 300ms
      if (dist < 5 && time < 300) {
  ...
  handleCardClick(clientX: number, clientY: number) {
    ...
    const x = clientY !== undefined ? clientX - rect.left : clientX;
    
    // Normalized device coordinate x (-0.5 to 0.5)
    const normalizedX = (x / rect.width) - 0.5;
    const clickXWorld = normalizedX * this.viewport.width;

    let closestMedia: Media | null = null;
    let minDistance = Infinity;

    this.medias.forEach(media => {
      const distance = Math.abs(clickXWorld - media.plane.position.x);
    ...
    if (closestMedia && minDistance < (closestMedia as Media).plane.scale.x / 2) {
      const originalIndex = (closestMedia as Media).index % this.originalLength;
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onCardClick]);
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag or scroll to navigate, click to open work."
    />
  );
}
